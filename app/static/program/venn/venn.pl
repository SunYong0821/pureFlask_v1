...
for my $out(keys %hash)
{
	...
	my $tmp=join"_and_",@a;
	$count{$tmp}++;
	open OUT,">$outdir/$tmp.venn.xls" if (-s $tmp);
	close OUT;
...
{
	my @name=map{join"_and_",sort(@$_)} combine($out,@database);
	for my $in(@name)
	{
...
elsif($number==3)
{
	...
	$svg->text('x',465,'y',270,'-cdata',$output{"1"},'text-anchor','middle','font-size',24,'style','fill: black');
	$svg->text('x',370,'y',380,'-cdata',$output{"2"},'text-anchor','middle','font-size',24,'style','fill: black');
	$svg->text('x',465,'y',420,'-cdata',$output{"3"},'text-anchor','middle','font-size',24,'style','fill: black');
	$svg->text('x',560,'y',380,'-cdata',$output{"4"},'text-anchor','middle','font-size',24,'style','fill: black');
	...
	$svg->text('x',595,'y',510,'-cdata',$output{"7"},'text-anchor','middle','font-size',24,'style','fill: black');
...
system("convert -density 200 $outdir/Venn.svg $outdir/Venn.png");
