source('/home/test/03.bar/labels2colors.R')
args <- commandArgs(trailingOnly = TRUE)
inFile <- args[1]
prefix <- args[2]
outdir<-args[3]
outFile <- paste(args[3],"/",args[2],"_bar.pdf",sep="")
...
if(tax_num < (20+2)){
    ...
    cols = cols_brewer[1:20]
    cols[(20+1):tax_num] = '#000000'
...
pdf(outFile,height=hei,width=pdf_width)
...
bar_plot <- function(lmat = lmat, lhei = lhei, lwid = lwid,bottom_mar = bottom_mar,data = data,
  ...
  headtitle<-paste(args[2],"Level Barplot",sep=" ")
  if(ncol(data) == 1){
    ...
    mtext(headtitle, side=3, line=1, cex=font_size*1.1,,font=2)
  }else if(ncol(data)<=10){
  ...
  legend("top", pch=15, col=cols, legend=c(attributes(sort(apply(data,1,sum)[-nrow(data)],decreasing=T))$names[1:20],"Other"), bty="n",
         pt.cex=pt.cex, cex=2, ncol=ncol, x.intersp=x.intersp, y.intersp=y.intersp,xpd=TRUE)
  }
