...
from app.tools.tools_forms import RevComForm, PoolingForm, SplitLaneForm, DEGForm, VolcanoForm, MAplotForm, EZCLForm, \
    ...
    FaslengthfilterForm, PatternspecieschooseForm
from . import tools
...
def splitlane():
    ...
    if form.validate_on_submit():
        ...
        script = f"python ./app/static/program/splitlane/splitlane.py {inputfile[0]} {form.lane.data} " \
            f"2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def volcano():
    ...
    if form.validate_on_submit():
        ...
        script = f"perl ./app/static/program/volcano/Volcano_plot.pl -i {inputfile[0]} -f {form.fc.data} " \
            f"-log2col {form.fccol.data} -pvalue {form.pq.data} -pCol {form.pqcol.data}" \
...
def ma_plot():
    ...
    if form.validate_on_submit():
        ...
        with open(f"{taskdir}/run.log", "w") as optfile:
            optfile.write(
                f"Options: {form.fc.data} {form.fccol.data} {form.pq.data} {form.pqcol.data} {form.exp1.data} "
                f"{form.exp2.data} {form.outpre.data}\n")
        script = f"perl ./app/static/program/ma_plot/MA_plot.pl -i {inputfile[0]} -log2col {form.fccol.data} " \
...
def venn():
    ...
    if form.validate_on_submit():
        ...
        script = f"perl ./app/static/program/venn/venn.pl -l {filestr} -h {form.head.data} -col {form.col.data} " \
            f"2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def edger():
    ...
    if form.validate_on_submit():
        ...
        script = f"perl ./app/static/program/edger/DiffExp_edgeR.pl -i {inputfile[0]} -count1col {form.exp1.data} " \
            f"-count2col {form.exp2.data} -genecol {form.gene.data} -bcv {form.bcv.data} -prefix {form.outpre.data}" \
            f" 2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def keggbublle():
    ...
    if form.validate_on_submit():
        ...
        with open(f"{taskdir}/run.log", "w") as optfile:
            optfile.write(
                f"Options: {form.pathcol.data} {form.genecol.data} {form.bgcol.data} "
                f"{form.pqcol.data} {form.outpre.data}\n")
        script = f"perl ./app/static/program/keggbublle/Pathway_EnrichFigure.pl -i {inputfile[0]} " \
            f"-pathcol {form.pathcol.data} -genecol {form.genecol.data} -background {form.bgcol.data} " \
            f"-pcol {form.pqcol.data} " \
            f"-prefix {form.outpre.data} 2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def clustertree():
    ...
    if form.validate_on_submit():
        ...
        script = f"perl ./app/static/program/clustertree/ClusterTree.pl -i {inputfile[0]} -expcol {form.expcol.data}" \
            f" -method {form.method.data} -prefix {form.outpre.data} -width {form.width.data}" \
            f" -height {form.height.data} 2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def pheatmap():
    ...
    if form.validate_on_submit():
        ...
        script = f"perl ./app/static/program/pheatmap/heatmap.pl -in {inputfile[0]} -namecol {form.namecol.data} " \
            f"-datacol {form.datacol.data} -prefix {form.outpre.data} -scale {form.scale.data} -width {form.width.data}" \
            f" -height {form.height.data} -cluster_rows {form.cluster_rows.data} -cluster_cols {form.cluster_cols.data}  " \
...
def correlation():
    ...
    if form.validate_on_submit():
        ...
        script = f"perl ./app/static/program/correlation/Correlation.pl -i {inputfile[0]} -exp1col {form.exp1.data}" \
    ...
    if form.validate_on_submit():
        ...
        with open(f"{taskdir}/run.log", "w") as optfile:
            optfile.write(
                ...
                f"{form.outpre.data}\n")
        script = f"perl ./app/static/program/fisher/Fisher.pl -i {inputfile[0]} -n11 {form.n11.data} " \
            f"-n12 {form.n12.data} -n21 {form.n21.data} -n22 {form.n22.data} -method {form.method.data} " \
            f"-prefix {form.outpre.data} 2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def convertp():
    ...
    if form.validate_on_submit():
        ...
        script = f"python ./app/static/program/convertp/convertpics.py {inputfile[0]} {form.method.data} " \
            f"{form.density.data} 2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
        return redirect(url_for("admin.index"))
...
def barbox():
    ...
    if form.validate_on_submit():
        ...
        with open(f"{taskdir}/run.log", "w") as optfile:
            optfile.write(
                f"Options: {form.log.data} {form.dcol.data} {form.title.data} {form.xlab.data} {form.ylab.data} "
                f"{form.outpre.data}\n")
        script = f"perl ./app/static/program/barbox/barbox.pl -in {in1} -group {in2} -dcol {form.dcol.data} " \
            f"-log {form.log.data} -title {form.title.data} " \
...
def gccount():
    ...
    if form.validate_on_submit():
        ...
        script = f"python ./app/static/program/gccount/GCcontent.py {inputfile[0]} {form.bin.data} {form.outpre.data}" \
            f" 2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def vcf2phylip():
    ...
    if form.validate_on_submit():
        ...
        script = f"python ./app/static/program/vcf2phylip/vcf2phylip.py {inputfile[0]} {form.outpre.data}" \
            f" 2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
def fastalength():
    ...
    if form.validate_on_submit():
        ...
        script = f"python ./app/static/program/fastalength/fastalength.py {inputfile[0]} {form.length.data} " \
            f"2>>{taskdir}/run.log"
        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
...
@tools.route('/pattern_species_choose.html', methods=['GET', 'POST'])
@login_required
def pattern_species_choose():
    form = PatternspecieschooseForm()
    tool = Toolslist.query.filter_by(url="tools.pattern_species_choose").first()
    if form.validate_on_submit():
        taskdir, uuid, inputfile = taskprepare("tools.pattern_species_choose", form.url.data)
        with open(f"{taskdir}/run.log", "w", encoding='utf-8') as optfile:
            optfile.write(
                f"Options:{form.url.data} {form.pattern_species.data} {form.head.data}\n")
        script = f"python ./app/static/program/species_fasta/pattern_of_species.py {inputfile[0]} " \
            f"{form.pattern_species.data} {form.head.data}  2>>{taskdir}/run.log"

        app = current_app._get_current_object()
        crun = threading.Thread(target=runtools, args=(app, script, uuid))
        crun.start()
        return redirect(url_for("admin.index"))
    return render_template('admin/tools/pattern_species_choose.html', form=form, tool=tool)
